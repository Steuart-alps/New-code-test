...
export default function SafeTrackPage() {
  ...
  const tabConfig = {
    ...
    inductions: { label: "Inductions", icon: Users, sub: "inductions", headers: ["Staff", "Start Date", "Status", "Site"], rows: indRows },
...
function InductionForm({ form, setForm }: { form: any; setForm: any }) {
  return <>
    ...
    <div className="grid grid-cols-2 gap-4">
      ...
      <F label="Completed Date"><Input type="date" value={form.completedAt ?? ""} onChange={e => setForm({ ...form, completedAt: e.target.value })} /></F>
    </div>
    <F label="Checklist / Tasks"><Textarea value={form.checklist ?? ""} onChange={e => setForm({ ...form, checklist: e.target.value })} rows={4} placeholder="One item per line…" /></F>
    <F label="Notes"><Textarea value={form.notes ?? ""} onChange={e => setForm({ ...form, notes: e.target.value })} rows={2} /></F>
  </>;
}
